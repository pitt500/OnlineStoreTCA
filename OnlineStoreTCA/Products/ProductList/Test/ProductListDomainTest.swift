...
class ProductListDomainTest: XCTestCase {
    ...
    func testResetProductsToZeroAfterPayingOrder() async {
        ...
        await store.send(\.cartButtonTapped) {
            $0.cartState = expectedCartState
        }
        
        await store.send(\.cart.view.didPressPayButton) {
            $0.cartState?.alert = .confirmationAlert(totalPriceString: "$0.0")
        }
        
        await store.send(\.cart.alert.success) {
            $0.cartState?.alert = nil
        }
        await store.receive(\.cart.delegate.orderFinished) {
            $0.cartState = nil
    ...
    func testItemRemovedFromCart() async {
        ...
        await store.send(\.cart.cartItem[id: id1].deleteCartItem, products[0]) {
            $0.cartState?.cartItems = []
        }
        
        await store.receive(\.cart.view.getTotalPrice) {
